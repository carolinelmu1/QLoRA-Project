...
**This project uses [Unsloth](https://github.com/unslothai/unsloth)**, an optimized library for LoRA/QLoRA fine-tuning (recommended by course TA). Unsloth provides:
- **2-5x faster** training than standard PEFT implementations
- **30-40% additional memory savings** beyond quantization
- **Optimized kernels** for efficient gradient computation
...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    max_seq_length=1024,
    load_in_4bit=False  # False = 16-bit
)
...
model, tokenizer = FastLanguageModel.from_pretrained(
    model_name="gpt2-medium",
    max_seq_length=1024,
    load_in_4bit=True  # True = 4-bit NF4 quantization
)
...
# Train with Unsloth's SFTTrainer (optimized for instruction tuning)
...
│
├── src/
│   ├── model_utils.py                # Model loading with Unsloth
│   ├── training.py                   # Training with Unsloth's SFTTrainer
...
Using Unsloth provided significant performance improvements:

| Metric | Standard PEFT | Unsloth | Improvement |
|--------|--------------|---------|-------------|
| Training Speed | [X]s/step | [Y]s/step | **[Z]x faster** |
| Memory Usage | [A] MB | [B] MB | **[C]% reduction** |
| Implementation | Manual | Optimized | Cleaner code |

**Key Takeaway:** Unsloth's optimized kernels delivered [TODO: FILL]x speedup while maintaining identical model quality, making it the recommended library for LoRA/QLoRA fine-tuning.
...
- **Unsloth:** [https://github.com/unslothai/unsloth](https://github.com/unslothai/unsloth) - **Primary library used** - Optimized LoRA/QLoRA (2-5x faster than PEFT)
...
- **PEFT:** [https://github.com/huggingface/peft](https://github.com/huggingface/peft) - Parameter-Efficient Fine-Tuning (underlying framework)
- **TRL:** [https://github.com/huggingface/trl](https://github.com/huggingface/trl) - Transformer Reinforcement Learning (SFTTrainer)
...
# Install Unsloth (Google Colab recommended)
pip install "unsloth[colab-new] @ git+https://github.com/unslothai/unsloth.git"

# Install additional dependencies
pip install datasets matplotlib seaborn pandas numpy scikit-learn tqdm
```
...
- **Unsloth Team** for the optimized LoRA/QLoRA library that made this project feasible
